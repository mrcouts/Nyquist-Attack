...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.2]{theadeptquat.jpg}  
...
%	Devido à grande dificuldade de se obter o modelo dinâmico completo de mecanismos paralelos, muitos pesquisadores preferem utilizar mecanismos seriais para realizar tarefas que exigem um grande domínio sobre a dinâmica dos sistema, como plataformas robóticas voltadas a reabilitação, pois é necessário um conhecimento detalhado do comportamento dinâmico do mecanismo utilizado para poder controlar as forças de interação entre o mecanismo e o paciente \cite{Andre}.
	...
	Atualmente novas metodologias para modelagem de dinâmica multicorpos que se mostram muito mais adequadas para aplicações em qualquer tipo de mecanismo estão sendo desenvolvidas \cite{Jarzebowska2009,Cheng, 20altuzarra}, das quais se destaca o trabalho realizado por Renato Orsino, doutorando também orientado pelo professor Dr. Tarcísio Coelho \cite{Apostila, Orsino2013, 21orsino, 22orsino, 23orsino}.
...
\end{itemize}
\item Elabora\c{c}\~ao de uma metodologia de projeto de controle n\~ao linear robusto, baseado na t\'ecnica de controle por modos deslizantes \cite{Slotini, Utkin}, para mecanismos de arquitetura paralela, tradicionais e com atua\c{c}\~ao redundante \cite{Cheng}, com incertezas param\'etricas.
...
a realização da modelagem e simulação cinemática e dinâmica do mecanismo 2\underline{R}SU + \underline{P}PaP \cite{Kumazawa}, o projeto de um controlador não linear multivariável robusto baseado no modelo dinâmico, e a validação experimental das leis de controle sintetizadas.
