...
\nomenclature[A051]{$\ssA, \ssB, \ldots$}{Sistemas ou subsistemas mec\^anicos multicorpos}

% special char

\nomenclature[CAA01]{$\underline{a}$}{Matriz diagonal cuja diagonal \'e composta pelos par\^ametros $a_1, a_2, \ldots , a_n$}
...
\nomenclature[CAC91]{$\ccos_i$}{Abrevia\c{c}\~ao de $\cos(\theta_i)$}
\nomenclature[CAC92]{$\ccos_{i \plus j}$}{Abrevia\c{c}\~ao de $\cos(\theta_i + \theta_j)$}

%\nomenclature[CAD11]{$\md_{n}$}{Dynamic invariants column-matrix}
\nomenclature[CAD91]{$\dd$}{Operador diferencial}
\nomenclature[CAD92]{$\diag(.)$}{Matriz coluna cujos elementos s\~ao a diagonal de uma matriz quadrada}
...
\nomenclature[CAP21]{$\vct{\ttp}_{\ttB}$}{Matriz-coluna formada pelas coordenadas do ponto $\ttp$ no sistema de coordenadas $\ttB$.}
\nomenclature[CAP22]{$\hvct{\ttp}_{\ttB}$}{Matriz-coluna formada pelas coordenadas do ponto $\ttp$ no sistema de coordenadas $\ttB$ em coordenadas homog\^eneas, ou seja: $\hvct{\ttp}_{\ttB} = \begin{bmatrix}
[\ttp]_{\ttB} \\
1
\end{bmatrix} $}
...
\nomenclature[CAS91]{$\ssin_i$}{Abrevia\c{c}\~ao de $\sin(\theta_i)$}
\nomenclature[CAS92]{$\ssin_{i \plus j}$}{Abrevia\c{c}\~ao de $\sin(\theta_i + \theta_j)$}
...
\nomenclature[CAV33]{$\vct{\mv}^\msD$}{Matriz diagonal cuja diagonal \'e composta pelos elementos da matriz coluna $\mv$}
...
\nomenclature[SM104]{$\vct{\varnothing}$}{Matriz coluna vazia ou matriz vazia}
