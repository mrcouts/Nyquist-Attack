...
f = simplify(Matrix([R.ph_, R.Mh_**-1 * (u - R.vh_ - R.gh_ )]).subs(PIrep)).evalf()
f2 = Matrix([f[i] for i in range(len(f))])
fn = lambda t,X:f2.subs([(x_[i],X[i]) for i in range(len(x_)-1,-1,-1)]).subs(symbols('t'),t).evalf()
