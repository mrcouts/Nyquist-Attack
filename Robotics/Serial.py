...
phi_ = po_ - Matrix([symbols('beta')*ph_[1], RR.vw_[0][0]+mot.ph_[0],RR.vw_[0][1],RR.vw_[0][2],RR.vv_[0][0],RR.vv_[0][1],RR.vv_[0][2],0*(symbols('gamma_1')*ph_[0]+symbols('gamma_2')*ph_[1]+symbols('gamma_3')*ph_[2]), RR.vw_[1][0],RR.vw_[1][1],RR.vw_[1][2]+mot2.ph_[0],RR.vv_[1][0],RR.vv_[1][1],RR.vv_[1][2]]).subs(RR.StaticBal)
Ah_ = phi_.jacobian(ph_)
Ao_ = phi_.jacobian(po_)
C_ = Matrix([eye(3),simplify(-Ao_**-1 * Ah_)])
Mh_ = simplify( (C_.T * M_ * C_).subs(symbols('Jy'+str(2+1) + '_' + str(0)), symbols('Jx'+str(2+1) + '_' + str(0))) )
vh_ = simplify(  (C_.T * ( M_ * C_.diff(t)*ph_ + v_ ) ).subs(symbols('Jy'+str(2+1) + '_' + str(0)), symbols('Jx'+str(2+1) + '_' + str(0))) )
